...
- **Package Vitest configs import nmr without declaring it.** `@williamthorsen/nmr/vitest` resolves from the root devDependency through pnpm's upward walk; the package manifests declare no devDependencies at all. Declaring it in all 16 would pin the version in 17 places for a dev-only import in a file that is never published (`files` is `["dist/*", "CHANGELOG.md"]`).
