...
import { isArraySubscript, isManifestSearch } from '../site-handoffs.ts';
...
describe(isManifestSearch, () => {
  it('claims a walk probing for a manifest', () => {
    expect(isManifestSearch(['package.json'])).toBe(true);
  });

  it('claims a walk probing for a manifest beside other markers', () => {
    expect(isManifestSearch(['.git', 'package.json', 'pnpm-workspace.yaml'])).toBe(true);
  });

  it('declines a walk probing for a repository marker alone', () => {
    expect(isManifestSearch(['.git'])).toBe(false);
  });
  ...
  it('declines a walk probing for a name that merely ends in the manifest name', () => {
    ...
    expect(names.filter((name) => isManifestSearch([name]))).toStrictEqual([]);
  });
  ...
  it('declines a walk that probes for no name', () => {
    expect(isManifestSearch([])).toBe(false);
  });
});
