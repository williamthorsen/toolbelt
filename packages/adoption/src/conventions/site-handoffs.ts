...
/**
 * Reports whether the names probed at one level of a directory walk make the walk a manifest search.
 *
 * Two kits recognize an ascent that probes each level for a name, and this question decides which one claims a
 * given site: A walk looking for `package.json` is `toolbelt.packaging`'s, reported by its kit, and
 ...
 * @internal
 */
export function isManifestSearch(names: readonly string[]): boolean {
