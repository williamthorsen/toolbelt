...
const entryPoint = new URL('../dist/esm/bin/tb-node.js', import.meta.url);
...
if (!existsSync(entryPoint)) {
  process.stderr.write(
    'tb-node: build output not found. In a source checkout, run `nmr build`; otherwise reinstall the package.\n',
  );
  process.exit(1);
}
...
try {
  ...
  process.stderr.write(`tb-node: failed to load: ${message}\n`);
  process.exit(1);
}
