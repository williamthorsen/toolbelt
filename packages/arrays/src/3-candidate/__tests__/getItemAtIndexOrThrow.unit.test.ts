...
describe(getItemAtIndexOrThrow, () => {
  ...
  it('resolves a negative index from the end', () => {
    expect(getItemAtIndexOrThrow(['a', 'b', 'c', 'd'], -1)).toBe('d');
    expect(getItemAtIndexOrThrow(['a', 'b', 'c', 'd'], -4)).toBe('a');
  });
  ...
  it('if the index is past the end, throws a RangeError naming the index and the length', () => {
    ...
    expect(throwingFn).toThrow(new RangeError('No value at index 4 of an array of length 4.'));
  });

  it('if a negative index reaches past the start, throws a RangeError', () => {
    const throwingFn = () => getItemAtIndexOrThrow(['a', 'b', 'c', 'd'], -5);

    expect(throwingFn).toThrow(new RangeError('No value at index -5 of an array of length 4.'));
  });
  ...
  it('if the index names a hole, throws a RangeError rather than returning undefined', () => {
    ...
    expect(throwingFn).toThrow(new RangeError('No value at index 1 of an array of length 3.'));
  });
