 ...
 *
 * `toolbelt.packaging` claims a walk that probes each level for `package.json`, which its own kit reports, so the
 * walk detector declines such a site. Both kits read that rule from `isManifestSearch` in `packages/adoption`,
 * which keeps a consumer installing both packages from seeing one loop reported twice under conflicting advice.
 *
...
const PACKAGING_README_URL = 'https://github.com/williamthorsen/toolbelt/tree/main/packages/packaging#readme';
