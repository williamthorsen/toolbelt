...
A capitalization is claimed where the same subject supplies both halves, as in `word.charAt(0).toUpperCase() + word.slice(1)`. The subscript, `substring`, and template-substitution variants are claimed too. A re-cased tail is not: `word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()` is a different transformation, and this package publishes nothing that performs it. Taking `capitalize` from the `charAt(0)` form is an exact substitution; from the subscript form it is a correction, since indexing an empty string throws where `capitalize` returns the empty string.
