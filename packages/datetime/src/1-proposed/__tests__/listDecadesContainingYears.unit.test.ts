...
import { listDecadesContainingYears } from '../listDecadesContainingYears.ts';

describe(listDecadesContainingYears, () => {
  it('returns a single decade for years within the same decade', () => {
    const input = [1_980, 1_981, 1_985, 1_989];

    const result = listDecadesContainingYears(input);
  ...
  it('returns an empty array for no input', () => {
    expect(listDecadesContainingYears([])).toStrictEqual([]);
  });
  ...
  it('throws a RangeError when any years are negative', () => {
    expect(() => listDecadesContainingYears([-1, 1_980])).toThrow(RangeError);
  });
});
