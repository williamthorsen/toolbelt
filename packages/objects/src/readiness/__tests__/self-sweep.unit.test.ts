...
describe(listObjectIdioms, () => {
  // Every fix text describes its idiom without writing it out, which keeps the kit off its own report:
  // A spelled-out idiom would be a site in the source and again in the bundle. readyup drops the compiled
  // bundle from its own sweep, and nothing in CI runs `rdy run --packages`, so this suite fails on an
  ...
  it('finds nothing in the sources describing what it looks for', () => {
    const findings = listSweptFiles().flatMap((file) =>
      listObjectIdioms(fs.readFileSync(file, 'utf8')).map((site) => `${path.basename(file)}:${site.line}`),
    );
