...
describe(listManifestSearchSites, () => {
  ...
  // inlines shared modules that the sweep also reaches, so an edit writing the walk as code in any of them puts the
  ...
  it('finds nothing in the sources describing what it looks for', () => {
    const findings = listSweptFiles().flatMap((file) =>
      listManifestSearchSites(fs.readFileSync(file, 'utf8')).map((site) => `${path.basename(file)}:${site.line}`),
    );
  ...
  it('sweeps the modules and the compiled kit alike', () => {
    ...
    expect(names).toContain('listManifestSearchSites.ts');
  });
});
