...
if (!existsSync(entryPoint)) {
  process.stderr.write(
    'tb-secret: build output not found. In a source checkout, run `nmr build`; otherwise reinstall the package.\n',
  );
  process.exit(1);
}
...
try {
  ...
  process.stderr.write(`tb-secret: failed to load: ${message}\n`);
  process.exit(1);
}
